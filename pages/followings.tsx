...
import { useCreatePost, useGetPostsOfFollowings } from "@/hooks/post";
import TwitterLayout from "@/components/TwitterLayout";
import { graphqlClient } from "@/client/api";
import { getPreSingedURLForPostQuery } from "@/graphql/query/post";
...
function followings() {
    ...
    const { posts } = useGetPostsOfFollowings();
            ...
            <TwitterLayout>
                <div className="nav flex items-center justify-center sticky top-0 h-[3.2rem] border-b border-white/20 backdrop-blur-md bg-black/60 z-10">
                    <div className="relative w-[50%] h-full hover:bg-white/10 cursor-pointer">
                        <Link href={"/"}>
                            <div className="flex items-center justify-center w-full h-full">
                                <p className={`font-semibold text-white/50`}>For You</p>
                            </div>
                        </Link>
                    </div>
                    <div className="w-[50%] h-full hover:bg-white/10 cursor-pointer">
                        ...
                        <div className="absolute h-[4px] w-[3.5rem] bg-blue-400 right-[20.4%] bottom-0 rounded-full"></div>
                    </div>
                </div>
                {
                    user &&
